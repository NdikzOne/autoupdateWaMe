...
const handler = async (m, {
    ...
    const randEmoji = () => pick(emojiSets);
    ...
    const positif = semuaNama.filter(([_, saldo]) => saldo > 0).map(([n, s]) => `${randEmoji()} ${n} ${s}`);
    const negatif = semuaNama.filter(([n, s]) => s < 0 && pernahLF[n]).map(([n, s]) => `${randEmoji()} ${n} ${s}`);
...
handler.command = ["depo"];
handler.category = "rekap";
handler.description = "Deposit saldo ke pemain";
