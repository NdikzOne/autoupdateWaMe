const handler = async (m, {
    ...
    // Range biaya (versi panjang kaya contoh kamu)
    const biayaRanges = [{
        min: 1,
        max: 9,
        biaya: 1
    ...
    for (let line of lines) {
        ...
        if (currentKategori === "kecil") {
            ...
            if (match) {
                ...
                // Kalau ada huruf (LF/P/A), angka akhir = angkaAwal - biaya
                // Kalau tidak ada, angka akhir = angkaAwal * 2 - biaya
    ...
    let teks = `*📊 𝐑𝐄𝐊𝐀𝐏 𝐊𝐄𝐂𝐈𝐋:*\n\n`
    teks += hasilKecil.length ? hasilKecil.join("\n") : "Tidak ada"
    teks += `\n\n💰 *Total Fee Admin:* ${totalFee}\n\n© ${env.ownerName}`

    return m.reply(teks)
}

handler.command = ["rekapk"]
handler.category = "rekap"
handler.description = "Rekap hasil kategori kecil saja"
